...
const GC_API_SERVER="http://localhost:3000";
const drawFive = () => {
    axios.get(GC_API_SERVER+'/api/robots/five')
        .then(res => {
...
const duel = () => {
    ...
    setTimeout(() => {
        axios.post(GC_API_SERVER+'/api/duel', {compDuo, playerDuo})
            .then(({data}) => {
...
const getPlayerStats = () => {
    axios.get(GC_API_SERVER+'/api/player')
...
const getAllBots = () => {
    axios.get(GC_API_SERVER+'/api/robots')
        .then(({data}) => {
